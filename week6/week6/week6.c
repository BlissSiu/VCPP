...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) 
{
	...
	case WM_KEYDOWN:
		...
		if (rect_user.left < rect_target.right &&
			rect_user.right > rect_target.left &&
			rect_user.top < rect_target.bottom &&
			rect_user.bottom > rect_target.top)
		{
			InvalidateRect(hwnd, NULL, TRUE);
			Crash = 1; // 충돌 발생
		}
